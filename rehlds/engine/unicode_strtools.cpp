...
bool Q_IsDeprecatedW(uchar32 wch)
{
...
//-----------------------------------------------------------------------------
static uchar32 *StripWhitespaceWorker(uchar32 *pwch, int cchLength, bool *pbStrippedWhitespace)
{
	// walk backwards from the end of the string, killing any whitespace
	*pbStrippedWhitespace = false;

	uchar32 *pwchEnd = pwch + cchLength;
	while (--pwchEnd >= pwch)
	{
...
uchar32 *StripUnprintableWorker(uchar32 *pwch, bool *pbStrippedAny)
{
	uchar32 *pwchSource = pwch;
	uchar32 *pwchDest = pwch;
	*pbStrippedAny = 0;

	while (*pwchSource)
	{
		uchar32 cc = *pwchSource;
...
qboolean Q_StripUnprintableAndSpace(char *pch)
{
	...
	int cubDest = (cch + 1) * sizeof(uchar32);
	uchar32 *pwch_alloced = (uchar32 *)malloc(cubDest);
	bStrippedAny = false;
	bStrippedWhitespace = false;
	int cwch = (unsigned int)Q_UTF8ToUTF32(pch, pwch_alloced, cubDest, _STRINGCONVERTFLAG_ASSERT) >> 1;
	uchar32 *pwch = StripUnprintableWorker(pwch_alloced, &bStrippedAny);
	pwch = StripWhitespaceWorker(pwch, cwch - 1, &bStrippedWhitespace);
	if (bStrippedWhitespace || bStrippedAny)
		Q_UTF32ToUTF8(pwch, pch, cch, STRINGCONVERT_ASSERT_REPLACE);
